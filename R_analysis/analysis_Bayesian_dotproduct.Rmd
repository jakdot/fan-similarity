...
```{r}
testing_raw <- read_csv("online_testing_all.csv")
...
actr$ACT_R_activation <- scale(actr$ACT_R_activation)
...
m1 <- lmer(log(rt) ~ pmi + (1|pp_num) + (1|fan_word), data=subset(merged_data, condition == "target"))
print(summary(m1))

m2 <- lmer(log(rt) ~ pmi + fan + (1|pp_num) + (1|fan_word), data=subset(merged_data, condition == "target"))
print(summary(m2))

m2.1 <- lmer(log(rt) ~ actr + fan + (1|pp_num) + (1|fan_word), data=subset(merged_data, condition == "target"))
print(summary(m2.1))

m3 <- lmer(log(rt) ~ pmi + actr + fan + (1|pp_num) + (1|fan_word), data=subset(merged_data, condition == "target"))
print(summary(m3))

resid(m1)
m4 <- lm(resid(m1) ~ fan , data=subset(merged_data, condition == "target"))
print(summary(m4))
...
```{r}
fan_together_full <- modeling_full(merged_data, "rt", "actr", adaptdelta=TRUE)
save(fan_together_full, file="../stan_models/fan_together_full_actr.Rdata")
